...
const PROXY_USER_AGENT = "ReplicAI-Proxy/1.0";
...
export const openai = new OpenAI({
  ...
  defaultHeaders: { "User-Agent": PROXY_USER_AGENT },
});
...
export function getOpenRouterReasoningDefault(
  ...
  // Same Bedrock guard as ensureMaxReasoningEffort — see comment there.
  if (isOpenRouterAnthropicModel(model)) return {};
  return { reasoning: { effort: "xhigh" } };
}

/**
 * Ensures all recognised reasoning models run at maximum quality.
 *
 * Strategy (multi-scenario reasoning parameter handling):
 *   A. No reasoning field at all              → inject { effort: "xhigh" }
 *   B. Empty reasoning / undefined effort     → set effort to "xhigh"
 *   C. max_tokens set but no/low effort       → set effort to "xhigh", preserve max_tokens
 *   D. Already { effort: "xhigh" }            → pass through unchanged
 *   E. { effort: "low" | "medium" | "high" }  → upgrade to "xhigh"
 *
 * OpenRouter normalises effort → max_tokens for providers that require it,
 * so using effort "xhigh" universally requests the maximum available budget.
 * Returns a new passThrough object — does not mutate the original.
 */
export function ensureMaxReasoningEffort(
  model: string,
  passThrough: Record<string, unknown>
): Record<string, unknown> {
  if (!isOpenRouterReasoningModel(model)) return passThrough;

  // ── Anthropic/Bedrock safety guard ──────────────────────────────────
  // Anthropic models routed through AWS Bedrock use `thinking.budget_tokens`,
  // NOT `reasoning.effort`.  OpenRouter's effort→budget_tokens normalisation
  // is unreliable on Bedrock, and injecting `reasoning.effort` causes HTTP 400.
  // Skip automatic injection for all Anthropic models — OR's native Anthropic
  // integration handles reasoning configuration via provider-specific params.
  // Non-Anthropic reasoning models (OpenAI o-series, GPT-5, DeepSeek R1, etc.)
  // continue to receive effort: "xhigh" as before.
  if (isOpenRouterAnthropicModel(model)) return passThrough;

  const reasoning = passThrough["reasoning"];

  // Case A: no reasoning field → inject full default
  if (reasoning === undefined || reasoning === null || reasoning === false) {
    return { ...passThrough, reasoning: { effort: "xhigh" } };
  }

  // Cases B–E: reasoning field exists (possibly empty or under-specified)
  if (typeof reasoning === "object" && !Array.isArray(reasoning)) {
    const r = reasoning as Record<string, unknown>;
    // Already at max with no conflicting max_tokens → no change needed (Case D fast path)
    if (r["effort"] === "xhigh" && !("max_tokens" in r)) return passThrough;
    // Set / upgrade effort to xhigh; preserve user's max_tokens if present
    // (sanitizeOpenRouterReasoning will strip max_tokens later if both conflict)
    const patched = { ...r, effort: "xhigh" };
    return { ...passThrough, reasoning: patched };
  }

  // reasoning is boolean true or some other truthy non-object → replace with proper default
  return { ...passThrough, reasoning: { effort: "xhigh" } };
}
...
export function autoInjectPromptCaching(opts: {
  ...
  const { key, value: cc } = getCacheControlForModel(opts.model);

  // Bail out if caller already set cache_control or cachePoint anywhere — respect manual control.
  const hasCC = (x: unknown): boolean =>
    !!(x !== null && typeof x === "object" &&
      (("cache_control" in (x as object) && (x as Record<string, unknown>)["cache_control"]) ||
       ("cachePoint" in (x as object) && (x as Record<string, unknown>)["cachePoint"])));
  const alreadyCached =
  ...
  if (typeof system === "string" && system.length > 0) {
    system = [{ type: "text", text: system, [key]: cc } as any];
  } else if (Array.isArray(system) && system.length > 0) {
    const last = system[system.length - 1];
    system = [...system.slice(0, -1), { ...last, [key]: cc } as any];
  }
    ...
    if (Array.isArray(target.content)) {
      ...
      if (insertAt >= 0) {
        const updated = blocks.map((b, i) =>
          i === insertAt
            ? ({ ...(b as unknown as Record<string, unknown>), [key]: cc } as Anthropic.ContentBlockParam)
            : b,
        );
        messages = [
          ...messages.slice(0, ti),
  ...
  if (tools && tools.length > 0) {
    ...
    tools = [
      ...tools.slice(0, -1),
      { ...(lastTool as unknown as Record<string, unknown>), [key]: cc } as Anthropic.Tool,
    ];
  }
...
export function isBedrockModel(model: string): boolean {
  // Based on aws.txt, Amazon Nova models use the cachePoint marker.
  // We limit this to amazon/ models to avoid interfering with Claude-on-Bedrock
  // which typically uses the unified cache_control field on OpenRouter.
  return model.startsWith("amazon/nova-");
}

export function isPromptCacheSupportedModel(model: string): boolean {
  return isOpenRouterAnthropicModel(model) || isBedrockModel(model) || /^google\/gemini-/.test(model);
}

function getCacheControlForModel(model: string): { key: string; value: any } {
  if (isBedrockModel(model)) {
    return { key: "cachePoint", value: { type: "default" } };
  }
  return { key: "cache_control", value: { type: "ephemeral", ttl: "1h" } };
}
...
function hasBlockLevelCacheControlInOpenAIMessages(messages: OAIMessage[]): boolean {
  return messages.some((message) => {
    ...
    return message.content.some((part) => {
      ...
      // Check for both Anthropic/Gemini and Bedrock block-level markers
      return (p["cache_control"] !== undefined && p["cache_control"] !== null) ||
             (p["cachePoint"] !== undefined && p["cachePoint"] !== null);
    });
  });
}
...
function cloneMessageContentWithInjectedCacheControl(
  ...
  // This function is ONLY called in the OpenRouter routing path (via
  // prepareOpenRouterCachingRequest).  OpenRouter always uses cache_control —
  // it internally converts to cachePoint for Bedrock endpoints.
  // getCacheControlForModel() would inject cachePoint for amazon/nova-* models,
  // which OpenRouter doesn't recognise, silently breaking caching.
  ...
  if (typeof message.content === "string") {
    ...
    return {
      ...message,
      content: [{ type: "text", text: message.content, cache_control: cc }],
    };
  }
  ...
  const nextContent = message.content.map((part, index) =>
    index === injectAt
      ? ({ ...(part as Record<string, unknown>), cache_control: cc } as OAIContentPart)
      : part,
  );
...
type OpenRouterPromptCacheStrategy =
  | "not_applicable"
  | "pass_through_top_level"     // top-level cache_control already set; skip per-block injection
  | "pass_through_existing_block"
  | "block"
  | "no_eligible_block";

export function prepareOpenRouterCachingRequest(messages: OAIMessage[], model: string): {
  messages: OAIMessage[];
  strategy: OpenRouterPromptCacheStrategy;
} {
  if (hasBlockLevelCacheControlInOpenAIMessages(messages)) {
    return { messages, strategy: "pass_through_existing_block" };
  }

  // Clone so we can inject multiple breakpoints without mutating the input.
  const result = messages.map((m) => m);
  let injectedCount = 0;

  // ── T1b: Stable System Prefix Breakpoint ────────────────────────────
  // The first system message contains the stable VCP system prefix (after
  // dynamic sinking stripped volatile content to the last user message).
  // Marking it with cache_control ensures this large prefix (~20-35K tokens)
  // is cached across requests.
  for (let i = 0; i < result.length; i++) {
    if (result[i].role !== "system") continue;
    const injected = cloneMessageContentWithInjectedCacheControl(result[i], model);
  ...
  // ── P2: Conversation History Tail Breakpoint ────────────────────────
  // After dynamic sinking, the LAST user message carries volatile context
  // (RAG/time/weather) and must NOT be cached. But everything before it —
  // the growing conversation history — is append-only and stable.
  //
  // Placing a second breakpoint on the PENULTIMATE message ensures the
  // entire prefix [system..N-2] is cached by Anthropic's prefix caching.
  // For a 200K token VCP conversation this eliminates ~150K+ tokens of
  // cache misses per request that the old single-breakpoint design wasted.
  //
  // Anthropic supports up to 4 breakpoints; we use 2 (T1b + P2).
  // Only inject if:
  //   - at least 3 messages (system + history + current user)
  //   - penultimate isn't a system (already handled by T1b)
  if (result.length >= 3) {
    const penultimateIdx = result.length - 2;
    const penMsg = result[penultimateIdx];
    if (penMsg.role !== "system") {
      const injected = cloneMessageContentWithInjectedCacheControl(penMsg, model);
      if (injected) {
        result[penultimateIdx] = injected;
        injectedCount++;
      }
    }
  }

  // Fallback: if no system message was found, try the first user message
  // after system messages (original single-breakpoint behavior).
  if (injectedCount === 0) {
    const firstNonSystemIndex = result.findIndex((m) => m.role !== "system");
    for (let i = Math.max(0, firstNonSystemIndex + 1); i < result.length; i++) {
  ...
  return {
    messages: result,
    strategy: injectedCount > 0 ? "block" : "no_eligible_block",
  };
}
...
function extractOpenRouterPromptCacheMetrics(
  ...
  cacheStrategy: OpenRouterPromptCacheStrategy,
): {
  model: string;
  cacheStrategy: OpenRouterPromptCacheStrategy;
  promptTokens?: number;
...
type GeminiFunctionResponsePart = { functionResponse: { name: string; response: Record<string, unknown> } };
type GeminiPart = GeminiTextPart | GeminiFunctionCallPart | GeminiFunctionResponsePart;
type GeminiContent = { role: "user" | "model"; parts: GeminiPart[] };

function oaiContentToGeminiParts(content: string | OAIContentPart[] | null): GeminiTextPart[] {
...
export function convertMessagesToGemini(messages: OAIMessage[]): {
  ...
  // First pass: build tool_call_id → function_name map for resolving tool result messages.
  // Gemini's functionResponse requires the function name, but OAI role="tool" only carries
  // tool_call_id; we resolve it by scanning all assistant tool_calls in the history.
  ...
  for (const msg of messages) {
    if (msg.role === "system") {
      ...
      // Text blocks (filter out empty strings — Gemini rejects empty text parts)
      const textParts = oaiContentToGeminiParts(msg.content).filter(p => p.text.length > 0);
      ...
      // Merge consecutive model turns (satisfies Gemini alternation rule)
      ...
      // OAI tool result → Gemini functionResponse (must be in a "user" role content)
      const fnName = toolCallIdToName[msg.tool_call_id ?? ""] ?? "unknown_function";
      const resultText = typeof msg.content === "string"
        ? msg.content
        : Array.isArray(msg.content)
          ? (msg.content as OAIContentPart[]).map(p => (p as { text?: string }).text ?? "").join("")
          : "";
      const fnResPart: GeminiFunctionResponsePart = {
        functionResponse: { name: fnName, response: { output: resultText } },
      };
      // Merge into an existing user turn if possible (multiple tool results in one turn)
      const last = contents[contents.length - 1];
      if (last && last.role === "user") {
        last.parts.push(fnResPart);
      } else {
        contents.push({ role: "user", parts: [fnResPart] });
      }

    } else {
      // user
      const parts = oaiContentToGeminiParts(msg.content);
      // Merge consecutive user turns to satisfy Gemini alternation rule
      const last = contents[contents.length - 1];
      if (last && last.role === "user") {
        last.parts.push(...parts);
      } else {
...
// ----------------------------------------------------------------------
// Tool conversion: OpenAI tools -> Gemini function declarations
// ----------------------------------------------------------------------

function convertToolsToGemini(tools: OAITool[]): Record<string, unknown>[] {
  return [{
    functionDeclarations: tools.map((t) => ({
      name: t.function.name,
      description: t.function.description ?? "",
      // Gemini accepts JSON Schema directly; pass parameters as-is.
      parameters: t.function.parameters ?? { type: "object", properties: {} },
    })),
  }];
}

function convertToolChoiceToGeminiConfig(tc: OAIToolChoice | undefined): Record<string, unknown> | undefined {
  if (!tc || tc === "none") return { functionCallingConfig: { mode: "NONE" } };
  if (tc === "auto") return undefined; // Gemini default is AUTO — omit toolConfig
...
async function handleGeminiStream(
  ...
  const { model, max_tokens, temperature, top_p, tools, tool_choice } = body;
  ...
  try {
    ...
    if (tools && tools.length > 0 && tool_choice !== "none") {
      ...
      const toolConfig = convertToolChoiceToGeminiConfig(tool_choice);
    ...
    for await (const chunk of stream) {
      // Iterate over candidate parts to handle both text and functionCall responses.
      // Gemini function calls arrive as complete objects (not streamed char-by-char).
      ...
      if (parts) {
        ...
        // Fallback for simple text-only chunks (no candidates wrapper)
        let text: string | undefined;
        try { text = chunk.text ?? undefined; } catch { /* safety/error block — skip */ }
    ...
    sseWrite(res, makeChunk(id, model, {}, hasToolCalls ? "tool_calls" : "stop"));
    sseWrite(res, {
...
async function handleGeminiNonStream(
  ...
  try {
    ...
    // Parse candidate parts to support both text and function call responses.
    const candidates = (response as unknown as Record<string, unknown>)["candidates"] as Array<Record<string, unknown>> | undefined;
    const parts = (candidates?.[0]?.["content"] as Record<string, unknown> | undefined)?.["parts"] as Array<Record<string, unknown>> | undefined;

    let text = "";
    const toolCallResults: Array<{ id: string; type: "function"; function: { name: string; arguments: string } }> = [];
      ...
      for (const part of parts) {
        if (typeof part["text"] === "string") {
          ...
          toolCallResults.push({
            id: `call_${Date.now()}_${toolCallResults.length}`,
            type: "function",
            function: {
              name: fc.name ?? "",
              arguments: JSON.stringify(fc.args ?? {}),
    ...
    const id = `chatcmpl-${Date.now()}`;
    const finishReason = toolCallResults.length > 0 ? "tool_calls" : "stop";
    const assistantMessage: Record<string, unknown> = {
      role: "assistant",
      content: text || null,
    };
    if (toolCallResults.length > 0) assistantMessage["tool_calls"] = toolCallResults;
...
async function handleOpenRouterStream(
  ...
  // ── VCP-aware Dynamic Sinking ──────────────────────────────────────
  // Sink volatile content (RAG/time/weather) from system prompt to the
  // last user message so the stable system prefix + conversation history
  // form a consistent cacheable prefix.
  const vcpSink = applyVCPDynamicSinking(resolvedMessages);
  const messagesForCache = vcpSink.messages;
  if (vcpSink.applied) {
    req.log.info(
      { model: body.model, staticLen: vcpSink.staticLength, dynamicLen: vcpSink.dynamicLength },
      "vcp dynamic sinking applied (stream) — volatile content sunk to last user message",
    );
  }

  // If dynamic sinking already set a top-level cache_control (T1 plan, 1h TTL),
  // skip per-block injection — adding per-block on top would downgrade TTL to 5 min
  // and create redundant explicit breakpoints alongside the automatic-caching entry.
  const hasTopLevelCacheControl = !!body.cache_control;
  const cachingPrep = isPromptCacheSupportedModel(body.model)
    ? (hasTopLevelCacheControl
        ? { messages: messagesForCache, strategy: "pass_through_top_level" as const }
        : prepareOpenRouterCachingRequest(messagesForCache, body.model))
    : { messages: messagesForCache, strategy: "not_applicable" as const };

  if (cachingPrep.strategy === "block") {
    req.log.info({ model: body.model, vcpSplit: vcpSink.applied }, "openrouter prompt cache strategy=block");
  } else if (cachingPrep.strategy === "pass_through_top_level") {
    req.log.info({ model: body.model }, "openrouter prompt cache strategy=pass_through_top_level (1h TTL)");
  } else if (cachingPrep.strategy === "pass_through_existing_block") {
    req.log.info({ model: body.model }, "openrouter prompt cache strategy=pass_through_existing_block");
  }
  ...
  // Ensure maximum reasoning effort + sanitize conflict
  let pt = ensureMaxReasoningEffort(body.model, passThrough as Record<string, unknown>);
  pt = sanitizeOpenRouterReasoning(pt);

  // ── Auto-inject Bedrock provider routing for Claude models ──────────
  // Forces OpenRouter to route through AWS Bedrock for better cache economics.
  // Merges with (rather than replaces) any client-supplied provider prefs
  // so zdr, data_collection, require_parameters etc. are preserved.
  if (isOpenRouterAnthropicModel(body.model) && !pt["cache_control"]) {
    const clientProvider = (pt["provider"] as Record<string, unknown> | undefined) ?? {};
    if (!clientProvider["order"]) {
      pt = {
        ...pt,
        provider: {
          ...clientProvider,
          order: ["amazon-bedrock", "anthropic"],
          allow_fallbacks: clientProvider["allow_fallbacks"] ?? true,
        },
      };
    }
  }

  // ── Anthropic extended-thinking param compatibility ─────────────────
  // Anthropic API rejects temperature (must be 1), top_p, and top_k when
  // thinking is enabled. We clamp temperature and strip others ONLY for
  // Anthropic models to avoid 400 errors while preserving user intent
  // for other reasoning models (e.g. DeepSeek R1).
  if (isOpenRouterAnthropicModel(body.model) && pt["reasoning"]) {
    if (pt["temperature"] !== undefined) pt["temperature"] = 1;
    delete pt["top_p"];
    delete pt["top_k"];
  }
  ...
  try {
    ...
    if (isPromptCacheSupportedModel(body.model)) {
...
async function handleOpenRouterNonStream(
  ...
  if (vcpSink.applied) {
    req.log.info(
      { model: body.model, staticLen: vcpSink.staticLength, dynamicLen: vcpSink.dynamicLength },
      "vcp dynamic sinking applied (non_stream)",
    );
  }

  // Same guard as stream handler: if top-level cache_control already set (T1 plan,
  // 1h TTL), skip per-block injection to avoid TTL downgrade and duplicate entries.
  const hasTopLevelCacheControl = !!body.cache_control;
  const cachingPrep = isPromptCacheSupportedModel(body.model)
    ? (hasTopLevelCacheControl
        ? { messages: messagesForCache, strategy: "pass_through_top_level" as const }
  ...
  // ── Auto-inject Bedrock provider routing for Claude models ──────────
  // Skip Bedrock injection when top-level cache_control is set (T1 plan).
  if (isOpenRouterAnthropicModel(body.model) && !pt["cache_control"]) {
    const clientProvider = (pt["provider"] as Record<string, unknown> | undefined) ?? {};
    if (!clientProvider["order"]) {
      pt = {
  ...
  // ── Anthropic extended-thinking param compatibility ─────────────────
  // (same logic as stream handler — see detailed comment there)
  if (isOpenRouterAnthropicModel(body.model) && pt["reasoning"]) {
    if (pt["temperature"] !== undefined) pt["temperature"] = 1;
    delete pt["top_p"];
    delete pt["top_k"];
...
// ----------------------------------------------------------------------
// VCP-aware Dynamic Sinking for OpenRouter routes
// ----------------------------------------------------------------------

/**
 * Detects VCP-format system messages and splits them into a stable prefix
 * (messages[0]) and volatile/dynamic content (messages[1]) for optimal
 * prompt caching on OpenRouter.
 *
 * Sinks volatile content (RAG blocks, timestamps, weather, etc.) from the
 * system prompt to the LAST user message, so that:
 *
 * 1. messages[0] (stable system prefix) stays identical across requests
 *    → OpenRouter Sticky Routing stable hash → same provider
 *    → cache_control breakpoint on prefix → consistent cache reads
 * 2. Conversation history (messages[1..N-2]) is append-only and stable
 *    → Anthropic prefix caching covers the growing history
 * 3. Only the last user message (messages[N-1]) changes per request
 *    — it carries both the new user input and the volatile dynamic context
 *    — this is always a cache miss but is relatively small
 *
 * Net effect: the entire conversation history becomes cacheable instead of
 * only the stable system prefix, dramatically reducing costs in long sessions.
 *
 * Non-VCP requests pass through unchanged (detected via VCP marker strings).
 * Already-processed requests (layered cache plan already stripped volatile)
 * also pass through unchanged since there's no volatile text left to sink.
 */
export function applyVCPDynamicSinking(messages: OAIMessage[]): {
  messages: OAIMessage[];
  applied: boolean;
  staticLength: number;
  dynamicLength: number;
} {
  // 1. Find the first system message
  const sysIdx = messages.findIndex((m) => m.role === "system");
  if (sysIdx < 0) return { messages, applied: false, staticLength: 0, dynamicLength: 0 };

  const sysMsg = messages[sysIdx];
  const content = flattenSystemTextContent(sysMsg.content);
  if (!content) return { messages, applied: false, staticLength: 0, dynamicLength: 0 };

  // 2. Detect VCP format — only process VCP system prompts
  const isVCP =
    content.includes("VCP_RAG_BLOCK") ||
    content.includes("vcp_fold") ||
    content.includes("<<<[TOOL_REQUEST]>>>") ||
    content.includes("VCP\u5143\u601D\u8003") || // VCP元思考
    content.includes("\u2500\u2500\u2500\u2500\u8BB0\u5FC6\u533A\u2500\u2500\u2500\u2500"); // ————记忆区————
  if (!isVCP) return { messages, applied: false, staticLength: content.length, dynamicLength: 0 };

  // 3. Analyse system layers using existing VCP-aware pattern rules
  //    (SYSTEM_LAYER_RULES already covers RAG blocks, timestamps, weather,
  //     memory sections, async results, etc.)
  const analysis = analyzeSystemLayers(content);
  const dynamicText = analysis.volatileText.trim();
  if (!dynamicText) {
    // No volatile content found — either already stripped by layered cache plan,
    // or the system prompt happens to be fully static this turn.
    return { messages, applied: false, staticLength: content.length, dynamicLength: 0 };
  }

  // 4. Build the stable system (volatile stripped)
  const staticContent = analysis.systemWithoutVolatile;

  // 5. Reconstruct: stable system stays at original position,
  //    dynamic content sinks to the LAST user message for optimal prefix caching.
  ...
  // Find the last user message (searching backwards)
  let lastUserIdx = -1;
  ...
  const DYNAMIC_SEPARATOR = "\n\n[VCP Dynamic Context \u2014 Auto Injected]\n";
  const DYNAMIC_END = "\n[/VCP Dynamic Context]";
  const dynamicPayload = DYNAMIC_SEPARATOR + dynamicText + DYNAMIC_END;

  let newMessages: OAIMessage[];
  if (lastUserIdx >= 0) {
    const lastUser = baseMessages[lastUserIdx];
    let mergedContent: OAIMessage["content"];

    if (typeof lastUser.content === "string") {
      // String content — simply append
      mergedContent = lastUser.content + dynamicPayload;
    } else if (Array.isArray(lastUser.content)) {
      // Array-form content blocks — append a text block
      mergedContent = [
        ...(lastUser.content as Array<Record<string, unknown>>),
        { type: "text", text: dynamicPayload },
      ];
    } else {
      // null or unexpected — use dynamic content as full content
      mergedContent = dynamicPayload.trimStart();
    }

    newMessages = baseMessages.map((msg, i) =>
      i === lastUserIdx ? { ...msg, content: mergedContent } : msg,
    );
  } else {
    // Fallback: no user message found — append a synthetic user message
    newMessages = [
      ...baseMessages,
      { role: "user", content: dynamicPayload.trimStart() },
    ];
  }
  ...
  return {
    ...
    applied: true,
    staticLength: staticContent.length,
    dynamicLength: dynamicText.length,
  };
}
...
function injectHistoryBreakpoint(msgs: OAIMessage[]): { messages: OAIMessage[], applied: boolean } {
  ...
  const newMsgs = [...msgs];
  // Match the 1h TTL used for T1/T2 system-level breakpoints so all cache
  // entries share the same expiry window, minimising redundant cache writes.
  const cc = { type: "ephemeral" as const, ttl: "1h" as const };
  ...
  try {
    ...
    if (skipDynamicSinkingForStickyRouting) {
      ...
      if (systemText) {
        ...
        // ── Provider-aware T1 suppression ──────────────────────────────────
        // OpenRouter docs: top-level cache_control forces Anthropic-only routing,
        // excluding Bedrock and Vertex endpoints.  When provider.order targets
        // Bedrock, downgrade T1 → T1b (block-level on stable system) so the
        // provider preference is honoured while still caching the stable prefix.
        const providerOrder = (body.provider as { order?: string[] } | undefined)?.order;
        const targetsBedrock = Array.isArray(providerOrder) &&
          providerOrder.some((p: string) => /bedrock/i.test(p));
        const canUseTopLevelCache = !targetsBedrock;

        if (stable) {
          if (canUseTopLevelCache) {
            // T1: top-level automatic caching — Anthropic provider only
            finalBody = { ...body, cache_control: body.cache_control ?? { type: "ephemeral", ttl: "1h" } };
            rewrittenSystemContent = sinkRes.system;
            cachePlan = "T1";
          } else {
            // T1b: Bedrock-safe — block-level breakpoint on the full stable system.
            // Same cache benefit as T1, but works across ALL OR providers.
            rewrittenSystemContent = [
              { type: "text", text: sinkRes.system, cache_control: { type: "ephemeral", ttl: "1h" } } as OAIContentPart,
            ];
            cachePlan = "T1b";
          }
        } else if (lcpResult) {
    ...
    if (!stream) {
      ...
      if (isOpenRouter) {
        let pt = ensureMaxReasoningEffort(finalBody.model, finalBody as Record<string, unknown>);
        pt = sanitizeOpenRouterReasoning(pt);
